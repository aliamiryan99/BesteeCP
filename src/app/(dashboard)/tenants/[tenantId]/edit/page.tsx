...
export default function EditTenantPage() {
  ...
  const existingTenantServicesQuery = useQuery(api.services.staffServices.listTenantServices, tenantId ? { tenantId } : "skip");
  ...
  useEffect(() => {
    if (existingTenantServicesQuery && !servicesDataLoaded) {
      setTenantServices(existingTenantServicesQuery.map((ts: any) => ({
        serviceId: ts.serviceId || "",
        modelId: ts.modelId,
        price: ts.price,
        duration: ts.duration,
      })).filter((ts: any) => ts.serviceId));
      setServicesDataLoaded(true);
    }
  }, [existingTenantServicesQuery, servicesDataLoaded]);
                ...
                <div className="mb-6 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/40">خدماتی که این شعبه ارائه می‌دهد را مشخص کنید. این لیست کاتالوگ پیش‌فرض شعبه است.</p>
                    </div>
                  </div>
                </div>
